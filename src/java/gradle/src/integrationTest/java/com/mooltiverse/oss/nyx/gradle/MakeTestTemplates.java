...
public class MakeTestTemplates {
    ...
    static class ConstructorTests extends com.mooltiverse.oss.nyx.command.MakeTestTemplates.ConstructorTests {}

    @Nested
    @DisplayName("Make state")
    @ExtendWith(GradleCommandInvocationContextProvider.class)
    static class StateTests extends com.mooltiverse.oss.nyx.command.MakeTestTemplates.StateTests {}

    @Disabled("Nyx tasks perform theit up-to-date check internally")
    @Nested
    @DisplayName("Make isUpToDate")
    @ExtendWith(GradleCommandInvocationContextProvider.class)
    static class UpToDateTests extends com.mooltiverse.oss.nyx.command.MakeTestTemplates.UpToDateTests {}

    @Nested
    @DisplayName("Make run")
    @ExtendWith(GradleCommandInvocationContextProvider.class)
    static class RunTests extends com.mooltiverse.oss.nyx.command.MakeTestTemplates.RunTests {}

    /**
     * Performs checks on the task business actions.
     */
    @Nested
    @DisplayName("MakeTask.Actions.execute")
    @ExtendWith(GradleCommandInvocationContextProvider.class)
    static class ActionTests {
        ...
        void noExceptionOnExecuteWithValidGitRepositoryInCustomDirectoryTest(Project project, @CommandSelector(Commands.MAKE) CommandProxy command, Script script)
        throws Exception {
        ...
        @Baseline(Scenario.INITIAL_COMMIT)
        void noExceptionOnExecuteWithValidGitRepositoryInWorkingDirectoryTest(@CommandSelector(Commands.MAKE) CommandProxy command, Script script)
            throws Exception {
            script.getWorkingDirectory().deleteOnExit();
            assertDoesNotThrow(() -> command.run());
        }
        ...
        @TestTemplate
        @DisplayName("MakeTask.getActions().execute() feeds the nyxState extra property")
        @Baseline(Scenario.INITIAL_COMMIT)
        void nyxStateExtraProperty(Project project, @CommandSelector(Commands.MAKE) CommandProxy command, Script script)
            throws Exception {
            script.getWorkingDirectory().deleteOnExit();
