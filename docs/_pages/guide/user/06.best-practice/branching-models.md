...
Nyx can support any strategy so you're not constrained at all into one or another strategy. Instead you have complete freedom in defining your own custom one. Here you can find some considerations about how to adopt the strategy of your choice. The only assumption here is that we assume Git is the version control system in use and so terms like *branch*, *commit*, *tag*, *merge* etc are to be interpreted in the Git domain.
...
A simple yet critical decision you need to make is about which branches are to be kept [*healthy*](https://martinfowler.com/articles/branching-patterns.html#healthy-branch) or *production ready* any time. By *healthy* we mean that there are no build or test errors preventing the pipelines to complete succesfully. By *production ready*, instead, we also mean that code could be released to production at any time as it's considered to be fully tested. You should have **at least** your [*mainline*](https://martinfowler.com/articles/branching-patterns.html#mainline) (`master` or `main`) always *healthy* or even *production ready* but as the workflow complexity grows you need more.

By the way, here we use `master` and `main` interchangeably.
...
The branching model also affects (and leverages) [**automation**]({{ site.baseurl }}{% link _pages/guide/user/06.best-practice/automation.md %}). For example you may need to decide how branches are related to **stages** in your **pipelines**, which tests to run for each, which **environments** are to be used to deploy your artifacts depending on the branch etc.

### Mainline only

The simplest strategy is [using a single branch](https://martinfowler.com/articles/branching-patterns.html#mainline-integration), usually `master` or `main`. This is also how any project starts. You basically don't use branches as every commit goes straight into the mainline (when local copies are pushed onto the central repository).

All branching models have a mainline as it's the bare minimum. Whether it's named `master` or `main` doesn't matter but you usually have only one of the two.
...
In other words, you need a better strategy.

### Multiple branches

Once you decided to use multiple branches you need to know which ones. Here's where the branching strategy kicks in. This is completely up to you but remember that **your branching model needs to reflect your workflow**.
...
Also because the defect to be fixed is usually tracked on the issue tracking platform of choice, the naming convention is very similar to *feature branches*, like for example `fix-5678`, `hotfix-5678`, `fix/5678`, `hotfix/5678`.
...
While the *mainline* is actually an integration branch itself, many teams use other integration branches named `develop` (or `development`), `integration`, `latest` etc with lower expectations than `master` in terms of confidence but higher merge frequency.

Integration branches are commonly based on the level of confidence of the code they merge and are periodically merged into the mainline or into other integration branches.

While in most cases one integration branch is enough you may need more than one as your workflow grows in complexity. Many think that teams should use *feature* branches **OR** *integration* branches, not both. You can actually use them in conjuction so that *feature* branches are merged into *integration* branches rather than the mainline.
...
This way your branches are long lived: they start when the release starts to be prepared (usually branching off the *mainline*) and finish when the release is no longer supported and they are merged back to the *mainline* (or be perpetual).
...
Examples of these branches are `production`, `rc` (`release-candidate`), `stage`, `test`, `development`, `alpha`, `beta`, `gamma` etc.
...
While the common practice is to have just one *mainline* (`master` or `main`) to store the primary, [*healthy*](https://martinfowler.com/articles/branching-patterns.html#healthy-branch) version of the project code, some others need multiple ones, like when using [flavor branches](#flavor-branches).
