...
layout: single
toc: true
permalink: /guide/user/best-practice/automation/
---
...
The `publish` task uploads artifacts and makes them available to your audience and this may happen even for those pipelines not ending with an actual release. The `release` task, on the other hand, performs some final activities to *seal* the release and make it official. The `release` task should not be used to *publish* anything with the only exception of the release itself (along with [changelog]({{ site.baseurl }}{% link _pages/guide/user/02.introduction/artifacts-and-publications.md %}#changelog) as the descriptive release manifest).

As already [mentioned](#build-scripts), `publish` and `release` are often condensed into one to work around the limitation of some release management not issuing the release version until the `release` execution but when using Nyx [the version number is issued in the early stages]({{ site.baseurl }}{% link _pages/guide/user/02.introduction/how-nyx-works.md %}) so you don't need to deal with this limitation.
...
In this section we mention the *publish* task as something other than the [release publication]({{ site.baseurl }}{% link _pages/guide/user/02.introduction/how-nyx-works.md %}). While Nyx does its own release publication, the *publish* task we mean here covers publishing artifacts like binaries, containers, libraries etc to target platforms like software registries.
{: .notice--info}
