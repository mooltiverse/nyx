...
* fetching informations from the commit history
...
* the [version scheme]({{ site.baseurl }}/reference/version-schemes/) to use (chances are you want to use [SemVer]({{ site.baseurl }}/reference/version-schemes/#semantic-versioning-semver))
* the [branching strategy]({{ site.baseurl }}/best-practice/branching-models/) to use. If you don't have one you can assume you're using the [mainline only]({{ site.baseurl }}/best-practice/branching-models/#mainline-only), which is the simplest one to start with
* the [release types]({{ site.baseurl }}/reference/release-types/) you want to manage and what is your desired workflow for each
* the [commit message conventions]({{ site.baseurl }}/reference/commit-message-conventions/) you have adopted to provide structured informations along with commits
