...
In this section you can find instructions to get started using the Docker image in minutes. Consider these your first steps based or a standard scenario but there is much more you can do and control with Nyx. For more on using the Docker image see [Using the Docker image](../introduction/usage.mdx#using-the-docker-image).
...
In case you want to pull it from the [GitHub Container Registry](https://mooltiverse.github.io/nyx/pkgs/container/nyx), use the GitHub's registry like:
...
The `nyx` executable within the container is executed by default passing the [`infer`](../introduction/how-nyx-works.mdx#infer) command so the above command line is equivalent to `docker run -it --rm -v /local/path/to/project:/project mooltiverse/nyx:latest infer`.

To run a different command, pass it on the command line. For example, to run the [publish](../introduction/how-nyx-works.mdx#publish) command, run the container as:
...
From here you can use Nyx just like from the [command line](../introduction/usage.mdx#using-the-command-line), with only a few caveats. Depending on the chosen (combination of) [configuration methods](../introduction/configuration-methods.mdx) you can pass options on the [command line](../introduction/configuration-methods.mdx#command-line-options), as one or more [configuration files](../introduction/configuration-methods.mdx#supported-file-grammars) or as [environment variables](../introduction/configuration-methods.mdx#environment-variables). When using environment variables you can pass them to the Docker container as one or more `-e` [flags](https://docs.docker.com/engine/reference/run/#env-environment-variables).
