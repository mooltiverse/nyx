...
You can have as many conventions as you want. You can use [presets](../configuration-presets/index.md) that come bundled with Nyx, override them or define your own from scratch.
...
Within the `commitMessageConventions` block you can define as many conventions as you want, each in its own separate block. The `name` identifies the convention so to define a brand new convention make sure you give it a `name` that was not already in use. If you use a `name` that was already defined for a convention then you are **overriding** an existing convention. Depending on the [configuration method](../introduction/configuration-methods.mdx) you use the `name` property might be defined inside or outside the block that configures a single convention.
...
| [`commitMessageConventions/<NAME>/bumpExpressions`](#bump-expressions) | [map](../introduction/configuration-methods.mdx#collections-of-objects) | `--commit-message-conventions-<NAME>-bumpExpressions-<IDENTIFIER>=<REGEX>` | `NYX_COMMIT_MESSAGE_CONVENTIONS_<NAME>_BUMP_EXPRESSIONS_<IDENTIFIER>=<REGEX>` | N/A                                        |
...
| Related state attributes  | <button class="button button--outline button--info">[newVersion](../state-reference/global-attributes.mdx#new-version)</button> <button class="button button--outline button--info">[releaseScope/significantCommits](../state-reference/release-scope.mdx#significant-commits)</button> <button class="button button--outline button--info">[scheme](../state-reference/global-attributes.mdx#scheme) [version](../state-reference/global-attributes.mdx#version)</button> |
...
Those commits in the release scope that successfully match one of these expressions are available in the [`significantCommits`](../state-reference/release-scope.mdx#significant-commits) list.
...
* the identifier names must comply with the configured version [`scheme`](global-options.mdx#scheme) (the above example is using [Semantic Versioning](../introduction/version-schemes.mdx#semantic-versioning-semver))
...
:::info
When configuring this map using command line options or environment variables you need to pass flattened values as documented [here](../introduction/configuration-methods.mdx#collections-of-objects). In this case you can pass each bump expression as a command line option like `--commit-message-conventions-<NAME>-bumpExpressions-<IDENTIFIER_NAME>=<REGEX>` or as an environment variable like `NYX_COMMIT_MESSAGE_CONVENTIONS_<NAME>_BUMP_EXPRESSIONS_<IDENTIFIER_NAME>=<REGEX>`.
:::
