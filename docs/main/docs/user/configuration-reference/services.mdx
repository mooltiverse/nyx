...
`BASE_URI` is meant to be used if you're using GitHub on a self hosted environment. If that's your case just pass the URI to your REST API endpoint here (i.e. `https://github.example.com/api/v3/`) otherwise, if you're using the public service, do not pass any value.
...
:::warning
The release type [description](release-types.mdx#description) must be set in order to publish releases to GitLab.
:::
