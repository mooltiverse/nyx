 ...
 * <br>
 * With more details, for every template this provider produces two contexts: one to run commands standalone,
 * the other to test them invoked from the {@link Nyx} class. This way a single test template can test the command
 * behavior in multiple contexts, without duplicating test code.
 * <br>
 * In order to use this in templates you need to register this provider as a test extension
 * declaratively in a test method, like:<br>
 * <pre>
 *   @TestTemplate
 ...
 * </pre>
 * <br>
 * This way you can have all of the fixture parameters properly injected.
 * <br>
 * Parameters that can be resolved by this extension are:<br>
 * - {@link Command}, defined by the {@link CommandSelector} and {@link Baseline} annotations
 * - {@link Script} and {@link Workbench} to be initialized with a predefined state, defined
 *   by the {@link Baseline} annotation, to be used on the parameter being injected
 * <br>
 * The {@link Baseline} can be shared when applied to methods or classes. In this case they will
 * yield to the same objects, unless overridden in more specific contexts.
 * 
 * @see TestTemplate
 */
public class CommandInvocationContextProvider implements TestTemplateInvocationContextProvider {
     ...
     */
    private static class StandaloneCommandInvocationContext implements TestTemplateInvocationContext {
        /**
        ...
        public String getDisplayName(int invocationIndex) {
            return Objects.isNull(displayName) ? "[standalone context] ".concat(Integer.toString(invocationIndex)) : "[standalone context] ".concat(displayName);
        }
     ...
     */
    private static class NyxCommandInvocationContext implements TestTemplateInvocationContext {
        /**
         * The display name passed in the constructor and also returned by {@link #getDisplayName(int)}.
         */
        private String displayName = null;
         ...
         * @param displayName the display name to be returned when {@link #getDisplayName(int)} is invoked.
         */
        NyxCommandInvocationContext(String displayName) {
            super();
            this.displayName = displayName;
        }
        ...
        @Override
        public String getDisplayName(int invocationIndex) {
            return Objects.isNull(displayName) ? "[nyx context] ".concat(Integer.toString(invocationIndex)) : "[nyx context] ".concat(displayName);
        }
    ...
    private static abstract class AbstractParameterResolver implements ParameterResolver {
         ...
         * script relatively to the given scope. If no previous instance is created, a new one is instantiated and stored for further use.
         * 
     ...
     */
    private static class BaselineParameterResolver extends AbstractParameterResolver {
        /**
         * Default constructor.
         */
        BaselineParameterResolver() {
            super(Script.class);
        }
     ...
     */
    private static class StandaloneCommandParameterResolver extends AbstractParameterResolver {
        /**
         * Default constructor.
         */
        StandaloneCommandParameterResolver() {
            super(Command.class);
        }
        ...
        public Command resolveParameter(ParameterContext parameterContext, ExtensionContext extensionContext)
            ...
            if (parameterContext.getParameter().getType().isAssignableFrom(Command.class)) {
                ...
                if (parameterContext.isAnnotated(CommandSelector.class)) {
                    commandName = parameterContext.findAnnotation(CommandSelector.class).get().value();
                }
     ...
     */
    private static class NyxCommandParameterResolver extends AbstractParameterResolver {
        /**
         * Default constructor.
         */
        NyxCommandParameterResolver() {
            super(Command.class);
        }
        ...
        public Command resolveParameter(ParameterContext parameterContext, ExtensionContext extensionContext)
            ...
            if (parameterContext.getParameter().getType().isAssignableFrom(Command.class)) {
                ...
                try {
                    return new NyxCommand(new Nyx(Git.open(script.getWorkingDirectory())), commandName);
                }
                catch (Exception e)
                {
                    throw new ParameterResolutionException(String.format("Cannot resolve parameter %s in %s because the command could not be instantiated", parameterContext.getParameter().getName(), parameterContext.getParameter().getDeclaringExecutable().getName()));
