...
public class CleanTestTemplates {
    ...
    static class ActionTests {
        ...
        void noExceptionOnExecuteWithValidGitRepositoryInCustomDirectoryTest(Project project, @CommandSelector(Commands.CLEAN) Command command, Script script)
        throws Exception {
        ...
        @Baseline(Scenario.INITIAL_COMMIT)
        void noExceptionOnExecuteWithValidGitRepositoryInWorkingDirectoryTest(@CommandSelector(Commands.CLEAN) Command command)
            throws Exception {
            assertDoesNotThrow(() -> command.run());
        }
    }
