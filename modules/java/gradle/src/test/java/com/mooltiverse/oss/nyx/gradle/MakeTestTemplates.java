...
public class MakeTestTemplates {
    ...
    static class ActionTests {
        ...
        void noExceptionOnExecuteWithValidGitRepositoryInCustomDirectoryTest(Project project, @CommandSelector(Commands.MAKE) Command command, Script script)
        throws Exception {
        ...
        @Baseline(Scenario.INITIAL_COMMIT)
        void noExceptionOnExecuteWithValidGitRepositoryInWorkingDirectoryTest(@CommandSelector(Commands.MAKE) Command command)
            throws Exception {
            assertDoesNotThrow(() -> command.run());
        }
    }
