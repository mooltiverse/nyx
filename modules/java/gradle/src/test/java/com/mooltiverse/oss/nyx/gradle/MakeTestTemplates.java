...
public class MakeTestTemplates {
    ...
    static class ActionTests {
        ...
        @Baseline(Scenario.INITIAL_COMMIT)
        void noExceptionOnExecuteWithValidGitRepositoryInWorkingDirectoryTest(@CommandSelector(Commands.MAKE) CommandProxy command)
            throws Exception {
            assertDoesNotThrow(() -> command.run());
        }
        ...
        @Baseline(Scenario.INITIAL_COMMIT)
        void nyxStateExtraProperty(Project project, @CommandSelector(Commands.MAKE) CommandProxy command)
            throws Exception {
