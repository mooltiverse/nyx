...
public class InferTestTemplates {
    ...
    static class ActionTests {
        ...
        void noExceptionOnExecuteWithValidGitRepositoryInCustomDirectoryTest(Project project, @CommandSelector(Commands.INFER) Command command, Script script)
        throws Exception {
        ...
        @Baseline(Scenario.INITIAL_COMMIT)
        void noExceptionOnExecuteWithValidGitRepositoryInWorkingDirectoryTest(@CommandSelector(Commands.INFER) Command command)
            throws Exception {
            assertDoesNotThrow(() -> command.run());
        }
    }
