...
public class ServiceFactory {
    ...
    public static GitRemoteService gitRemoteServiceInstance(Provider provider, Map<String,String> options) {
        Service instance = instance(provider, options);
        if (instance.supports(Feature.GIT_REMOTE))
            try {
                return GitRemoteService.class.cast(instance);
            }
            catch (ClassCastException cce) {
                throw new UnsupportedOperationException(String.format("The %s provider supports the %s feature but instances do not implement the %s interface", provider.toString(), Feature.GIT_REMOTE.toString(), GitRemoteService.class.getName()), cce);
            }
        else throw new UnsupportedOperationException(String.format("The %s provider does not support the %s feature", provider.toString(), Feature.GIT_REMOTE.toString()));
    }
