...
public enum Scenario {
     ...
     * </pre>
     */
    INITIAL_COMMIT( f -> Script.from(f).andAddFiles()
     ...
     * </pre>
     */
    INITIAL_VERSION( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
     ...
     * </pre>
     */
    ONE_BRANCH_SHORT( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
     ...
     * </pre>
     */
    ONE_BRANCH_SHORT_CONVENTIONAL_COMMITS( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
        .andCommitWithTag("0.0.1")
     ...
     * </pre>
     */
    ONE_BRANCH_WITH_OVERLAPPING_TAGS( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
        .andCommitWithTag("0.0.1")
        .andCommitWithTag("0.0.2", "Annotated tag to commit 0.0.2")
        .andCommitWithTag("0.0.3")
     ...
     * </pre>
     */
    TWO_BRANCH_SHORT_UNMERGED( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
     ...
     * </pre>
     */
    TWO_BRANCH_SHORT_MERGED( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
        .andCommitWithTagInBranch("master", "0.0.1")
        .andCommitWithTagInBranch("master", "0.0.2", "Annotated tag to commit 0.0.2")
        .andCommitWithTagInBranch("master", "0.0.3")
     ...
     * </pre>
     */
    FIVE_BRANCH_UNMERGED_BUMPING( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
     ...
     * </pre>
     */
    FIVE_BRANCH_UNMERGED_BUMPING_COLLAPSED( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
        .inBranch("master")  .andAddFiles().andCommit("").andTag("0.0.1", null)
        .inBranch("master")
     ...
     * </pre>
     */
    EXTENDED_PRESET_BRANCHES_SHORT_UNMERGED( f -> Script.from(f).andAddFiles()
        .andCommit("Initial commit")
        .inBranch("master")
     ...
     * The returned script can be used to inspect the repository or perform further actions.
     * 
     * @param bare if {@code true} a bare repository is created, otherwise a regular repository
     * is initialized
     * 
     * @return the script used to realize the scenario. It can be used for further operations.
     * 
     * @throws Exception in case of any issue.
     */
    public Script realize(boolean bare)
        throws Exception {
        // create a new repository in a new temporary directory and apply the script
        if (bare)
            return function.apply(Script.bare().getGitDirectory()); // use the GitDirectory as the WorkingDirectory is not available for bare repositories
        else return function.apply(Script.fromScratch().getWorkingDirectory());
    }
