...
func TestNewActionWith(t *testing.T) {
	identity := NewIdentityWith("John", "jdoe@example.com")
	timeStamp := NewTimeStampFrom(time.Now())

	action := NewActionWith(*identity, *timeStamp)

	assert.Equal(t, *identity, action.GetIdentity())

	assert.Equal(t, *timeStamp, action.GetTimeStamp())
}
